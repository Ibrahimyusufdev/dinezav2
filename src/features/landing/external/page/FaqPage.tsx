...
const AccordionItem = ({ item, isOpen, onToggle }: AccordionItemProps) => (
  ...
  >
    ...
    >
      <span className="text-foreground text-sm leading-snug font-medium">{item.question}</span>
      <ChevronDown
        size={18}
    ...
    <>
      ...
      <section className="bg-brand-dark py-20 md:py-28">
        <div className="mx-auto max-w-3xl px-5 text-center md:px-10">
          ...
          <h1
            className="mb-5 leading-tight font-medium text-white"
      ...
      <section className="bg-brand-primary-light border-border border-t py-16">
        <div className="mx-auto max-w-xl px-5 text-center">
          <h2 className="text-foreground mb-3 text-3xl font-medium">Still have questions?</h2>
          <p className="text-muted-foreground mb-8">
            Our support team is always happy to help. Reach out anytime.
          </p>
          <Link
            to={EXTERNAL_LINKS.CONTACT_US}
            className="bg-brand-primary inline-flex items-center gap-2 rounded-full px-8 py-4 text-sm font-medium text-white transition-all hover:opacity-90"
          >
            Contact Support
          </Link>
        </div>
